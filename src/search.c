...
int search_file_rec_block ( int fd, struct ext3_super_block *sb, 
	...
	if ( block_num > 0 ) {
		...
		if ( lret < 0 ) {
		    perror ("ext3Viewer error - erreur 4 de lseek search");
		    return -1;
		}
		sret = read ( fd, &iblock_num, 4 );
		if ( sret < 0 ) {
		    perror ("ext3Viewer error - erreur 2 de lecture iblock search");
		    return -1;
...
int search_inode_rec_block ( int fd, struct ext3_super_block *sb, 
	...
	lret = lseek( fd, ((off_t) block_num)*EXT3_BLOCK_SIZE(sb), SEEK_SET);
	if ( lret < 0 ) {
	    perror ("ext3Viewer error - erreur de lseek iname");
	    return -1;
	}

	sret = read( fd, &dir_entry, sizeof( struct ext3_dir_entry_2 ) );
	if ( sret < 0 ) {
	    perror ("ext3Viewer error - erreur de lecture dir_entry iname");
	    return -1;
	}
	...
	{  
		...
		push ( &path, file_name, dir_entry.inode );
		ret = search_inode_rec ( fd, sb, dir_entry.inode, i_tofind, path, path_name );
		if ( ret == -1 ) 
		  return -1;
	    ...
	    lret = lseek( fd, ((off_t) block_num)*EXT3_BLOCK_SIZE(sb)+
			  next_dir_entry, SEEK_SET);
	    if ( lret < 0 ) {
		perror ("ext3Viewer error - erreur 2 de lseek iname");
		return -1;
	    }

	    sret = read( fd, &dir_entry, sizeof( struct ext3_dir_entry_2 ) );
	    if (sret == 0) {
		perror("ext3Viewer error - erreur 2 de lecture de dir_entry iname");
		exit(1);
	    }
	...
	if ( block_num > 0 ) {
	    ...
	    lret = lseek( fd, ((off_t) block_num)*EXT3_BLOCK_SIZE(sb), SEEK_SET);
	    if ( lret < 0 ) {
		perror ("ext3Viewer error - erreur 3 de lseek iname");
		return -1;
	    }
	    ...
	    sret = read ( fd, &iblock_num, 4 ); /*we read the block number of the 
						  indexed block */
	    if ( sret < 0 ) {
		perror ("ext3Viewer error - erreur de lecture iblock iname");
		return -1;
	    }
		...
		//pour chaque block on l'affiche ou on le parcours si c une table
		ret = search_inode_rec_block ( fd, sb, i_tofind, iblock_num, rec_level-1, path, path_name );
		if ( ret == -1 ) 
		  return -1;
		...
		/* on se deplace de 4 en 4octets car c'est la taille de u_32 sur
		   laquelle on stoque un numero de block */
		if ( lret < 0 ) {
		    perror ("ext3Viewer error - erreur 4 de lseek iname");
		    return -1;
		}
		sret = read ( fd, &iblock_num, 4 );
		if ( sret < 0 ) {
		    perror ("ext3Viewer error - erreur 2 de lecture iblock iname");
		    return -1;
		}
