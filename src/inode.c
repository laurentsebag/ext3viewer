...
void read_inode( int fd, struct ext3_super_block *sb,
  ...
  /* pour plus d'explications sur la lecure d'un inode voir la documentation
     developpeur
  */
  ...
  off_t lret; /* valeur de retour pour tester lseek et read */
  size_t sret;
  ...
  if( lret <= 0 ) {
    perror("ext3Viewer error - erreur de lseek inode");
    exit(lret);
  }
  /* on se place au debut de la table des inodes grace a gb_inode_table
     puis on decale de i_num_g inodes
  */

  sret = read( fd, i, sizeof(struct ext3_inode) );

  if( sret <= 0 ) {
    perror("ext3Viewer error - erreur de lecture inode");
    exit(1);
  }
  //  printf("@@@@@@2%u\n",num);
...
//i_size_high is a macro to i_dir_acl
	real_size=(__u64)(in->i_size)+diracl;//We mask the highest order bit of i_size since it's not used, and we "concatenate" the rest of the size contained in i_dir_acl to the real size of the file.
  ...
  */
  ...
  if ( rec_level == 0 ) {
	...
	*/
	...
	sret = read( fd, &dir_entry, sizeof( struct ext3_dir_entry_2 ) );
	if (sret == 0) {
	  perror("ext3Viewer error - erreur de lecture de dir_entry "
		 "print_dir_blocks");
	  exit(1);
	}
      }
...
{/*This function serves as a mask for value*/
    if((value&macro)==macro)//If value== 110010 and Macro == 000010, then value has the functionnality described in macro.    
{
	return 1;
    }
    else
...
void print_binary(const __u32 value,int size)//Print a number in binary form, 001010 style
...
void print_mountoptions(__u32 s_default_mount_opts)
{
    ...
    {
	printf("Desactivation of the 32-bits mode for UIDs and GIDs (retrocompatibility with older kernels).\n");
    }
...
void print_filemode(__u16 i_mode)
{
    ...
    */
    ...
    if(mask(i_mode,0x800))//From http://uranus.it.swin.edu.au/~jn/explore2fs/es2fs.htm
    {
	printf("SUID\n");
    }
...
void print_fileflags(__u32 i_flags)//The flag set has to be 'masked'
{
    ...
    {
	printf("File is in appending mode, writing is made at the end of the file.\n");
    }
