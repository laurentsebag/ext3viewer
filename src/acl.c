...
{ // Reads an entry of an inode extended attribute, given the superblock, the
  // block number, and an empty ext3_xattr_entry structure
  int ret;
...
{ // This function only prints extended attributes.
  /* ACLs are implemented under ext3 using extended attributes: see kernel
   * source http://lxr.linux.no/source/fs/ext3/xattr.h
   */
