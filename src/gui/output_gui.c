...
void print_superblock_gui ( ) {
  ...
  if ( !fs.isopen ) {
	...
	else//Mettre les else: obligatoire pour detecter le mode du journal
	  {
...
void print_imap () { 
  ...
  if ( !fs.isopen ) {
	...
	if(i%6==0)
	{
	    ...
	    //On regarde le numéro d'inode dans le groupe, on veut retrouver le numéro d'inode véritable: on ajoute le nombre d'inodes par groupe* le numero du groupe
...
void print_bmap () { 
  ...
  if ( !fs.isopen ) {
	...
	if(i%6==0)
	{
	    ...
	    //On regarde le numéro d'block dans le groupe, on veut retrouver le numéro d'block véritable: on ajoute le nombre d'blocks par groupe* le numero du groupe
...
void print_journal () {
  ...
  if ( !fs.isopen ) {
    ...
    gtk_text_buffer_insert_with_tags_by_name ( buffer, &iter, "Dumping the internal journal\n\n", -1, "title", NULL);


    gtk_text_buffer_insert_with_tags_by_name ( buffer, &iter, "The journal (jbd) takes note of each writing operation. When the filesystem is mounted, the flag s_feature_incompat is set to 1. If any problem occurs when the filesystem is mounted (eg. power failure), when it is remounted, the \"mount flag\" value is still set to 1 (mounted). Then the journal recovery	must be done. When a correct unmount operation is done, the \"mount flag\" is set to 0.\n\n", -1, "citalic", NULL);
