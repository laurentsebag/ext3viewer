...
void read_group_desc(int fd, const struct ext3_super_block *sb,
  ...
  if ( lret <= 0) {
    perror("ext3Viewer error - lseek error group_desc");
    exit(1);
  }
  ...
  if( sret <= 0 ) {
    perror("ext3Viewer error - error reading group_desc");
    exit(1);
  }
    ...
    char *datablockbitmap, __u16 gb_num)
{
  ...
  while ( b < bmax )
  {
    ...
    print_t_bin(datablockbitmap[b]);
    // /!\ counter starts from 1; so the lastnumber is never used, should not
    // be printed?
...
{
  // We want to retrieve the true block number, from the block number in the
  // group:
